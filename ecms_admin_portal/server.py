...
class App(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            logging.error(
                "ERROR(APP) : Could not find device db settings file, " + str(err)
            )
    ...
    def json_response(self, json_data, status_code=200):
        try:
            response = Response(
                json.dumps(json_data), mimetype="application/html", status=status_code
            )
            response.headers["Cache-Control"] = "no-cache, no-store, must-revalidate"
            response.headers["Pragma"] = "no-cache"
            response.headers["X-Robots-Tag"] = "noindex"
        ...
        try:
            ...
            if request.session is not None:
                if "sess_modified" in request.session:
                    sess_modified = datetime.strptime(
                        request.session["sess_modified"], "%Y-%m-%d %H:%M:%S.%f"
                    )
                    last_time = datetime.now() - timedelta(
                        hours=0, minutes=int(self.config["SESSION_TIMEOUT"])
                    )
            ...
            if sid is None and request.session["sid"] != None:
                resp.set_cookie(
                    "session_data", request.session["sid"], samesite="Strict"
                )
